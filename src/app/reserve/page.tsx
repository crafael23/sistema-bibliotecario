...
export default function ReservePage() {
  return (
    <div>
      <h1 className="text-2xl font-bold mb-4">Book Reservation</h1>
      <div className="mb-4">
        <Label htmlFor="search">Search Books</Label>
        <Input id="search" placeholder="Enter book title or author" className="max-w-sm" />
      </div>
      <Table>
        <TableHeader>
          <TableRow>
        ...
        <TableBody>
          {books.map((book) => (
            <TableRow key={book.id}>
              ...
              <TableCell>
                <Button disabled={!book.available}>{book.available ? "Reserve" : "Join Waitlist"}</Button>
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
